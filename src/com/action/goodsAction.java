...
public class goodsAction extends ActionSupport
{
	...
	public String goodsDelByMe1()
	{
		TGoods goods=goodsDAO.findById(goodsId);
		goods.setGoodsDel("no1");
		goodsDAO.attachDirty(goods);
		this.setMessage("成功");
		this.setPath("goodsManaByMe.action");
		return "succeed";
	...
	public String goodsManaByMe()
	{
		...
		String sql="from TGoods where goodsDel <> 'yes' and goodsUserId="+user.getUserId();
	...
	{
		String sql="from TGoods where goodsDel <> 'yes'";
		List goodsList=goodsDAO.getHibernateTemplate().find(sql);
		Map request=(Map)ServletActionContext.getContext().get("request");
		request.put("goodsList", goodsList);
		return ActionSupport.SUCCESS;
	...
	public String goodsNoTejiaAdd()
	{
		...
		String goodShijian = format.format(new Date()); 
		goods.setGoodsShijian(goodShijian);
		goods.setGoodsUserId(1);
		goods.setGoodsDel("no");
	...
	public String goodSearch()
	{
        Map request=(Map)ServletActionContext.getContext().get("request");
		String sql="";
		sql="from TGoods where goodsDel <>'yes' and goodsName like '%"+goodsName.trim()+"%'"+" order by goodsCatelogId";
